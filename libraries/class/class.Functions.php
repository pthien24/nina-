...
class Functions
{
    ...
    //**************************************** function them --------------------------------*/
        ...
        /* Get Product Item */
    public function GetProducts( $arr = array(), $class = '', $isLazy = true,$thumbsp = '260x260x1') {
        ...
        foreach ($arr as $k => $v) { 
            $strpro .='<div class="product">';
                ...
                $strpro .='<a class="box-product text-decoration-none" href="'.$v[$sluglang].'" title="'.$v['name' . $lang].'">
                    ...
                    if($v['photo2'] == '' ){
                        $strpro .=' <p class="pic-product2 scale-img">
                        '.$this->getImage(['sizes' => ''.$thumbsp.'', 'isWatermark' => WATERMARKPRODUCT, 'prefix' => 'product', 'upload' => UPLOAD_PRODUCT_L, 'image' => $v['photo'], 'alt' => $v['name' . $lang],'isLazy'=>$isLazy]).'
                        </p>';
                    }else{
                        $strpro .=' <p class="pic-product2 scale-img">
                    ...
                    $strpro .=' <h3 class="name-product text-split">'.$v['name' . $lang].'</h3>
                    <p class="price-product">';
                        if ($v['sale_price']) {
                            ...
                            if($v['regular_price']!=0) $giapro = $this->formatMoney($v['regular_price']); else $giapro = lienhe; //'<b>'.lienhe.'</b>: '.$optsetting['hotline']
                            $strpro .='<span class="price-new">'.$giapro.'</span>';
                        }
                    $strpro .='</p>
                </a>';
                if(CARTSITE==true){ 
                $strpro .='<p class="cart-product w-clear">
        ...
        function lay_baiviet($truyvan,$cssitem,$thumb,$lazyload='lazy'){
                ...
                foreach($truyvan as $k => $v){ 
                    $str .='<a class="'.$cssitem.' text-decoration-none w-clear" href="'.$v[$sluglang].'" title="'.$v['name'.$lang].'">
                        <p class="pic-news scale-img">'.$this->getImage(['class' => $lazyload.' w-100', 'sizes' => $thumb, 'upload' => UPLOAD_NEWS_L, 'image' => $v['photo'], 'alt' => $v['name' . $lang]]).'</p>
                        <div class="info-news">
                            <h3 class="name-news">'.$v['name'.$lang].'</h3>
        ...
        function get_product($dieukien,$typesp,$gioihan){
            global $lang;
            $sql="select type, id, name$lang, slugvi, slugen, desc$lang, content$lang, code,    view, id_brand,  id_list, id_cat, id_item, id_sub,  photo, photo2, options,  sale_price, regular_price from #_product where type='".$typesp."'  and find_in_set('hienthi',status) and  find_in_set('".$dieukien."',status) order by numb asc limit 0,".$gioihan;
            $arr_cap=array();           
            return $truyvan = $this->d->rawQuery($sql,$arr_cap);            
        }
