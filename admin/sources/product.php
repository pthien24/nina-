    ...
    if ($data) {
        if($_POST['code']) {
            foreach ($_POST['code'] as $key => $value) {
                $dataVariants = array();
                $dataVariants['id_parent'] = ($_POST['id_parent'.$value]) ? $_POST['id_parent'.$value] : 0;
                $dataVariants['color'] = ($_POST['color'.$value]) ? $_POST['color'.$value] : 0;
                $var_check = $d->rawQueryOne("select id,photo from table_variants where id_parent = ? and color = ? limit 0,1",array($_POST['id_parent'.$value], $_POST['color'.$value]));
                if($var_check) {
                ...
                if($var_check) {
                    ...
                    $d->update('variants', $dataVariants);
                }
                else {
                    $d->insert('variants', $dataVariants);
                }
            }
        }
        if($_POST['code1']) {
            foreach ($_POST['code1'] as $key => $value1) {
                ...
                $var_check1 = $d->rawQueryOne("select id,price_new from table_variants1 where id_parent = ? and size = ? limit 0,1",array($_POST['id_parent'.$value1], $_POST['size'.$value1]));
                ...
                if($var_check1) {
                    ...
                    $d->update('variants1', $dataVariants1);
                }
                else {
                    $d->insert('variants1', $dataVariants1);
                }
            }
        }
